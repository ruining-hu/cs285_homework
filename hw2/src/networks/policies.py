...
class MLPPolicy(nn.Module):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        action = self.forward(obs=ptu.from_numpy(obs)).sample()
    ...
    def forward(self, obs: torch.FloatTensor):
        ...
        if self.discrete:
            ...
            return D.Normal(loc=self.mean_net(obs), scale=torch.exp(self.logstd))
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(
        ...
        if self.discrete:
            log_probs = self.forward(obs=obs).log_prob(actions)
        else:
            log_probs = self.forward(obs=obs).log_prob(actions).sum(dim=-1)
